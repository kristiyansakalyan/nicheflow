...
base_ckpt_path = Path("ckpts/kregion_ablations")
base_output_path = Path("outputs/eval_kregion_ablations")
model = "nicheflow"
variant = "glvfm"
datasets = ["med", "abd", "mba"]
# 64 is already used in the main experiments
k_regions = [8, 16, 32]
model_to_ckpt = {"nicheflow": "NicheFlow"}

# Evaluation
eval_runs = 5


def evaluate(experiment_override: str, k_regions_override: int, ckpt_path: Path):
    config = compose(
        config_name="train",
        overrides=[
            f"experiment={experiment_override}",
            f"data.datamodule.k_regions={k_regions_override}",
        ],
    )
    ...
    df.to_csv(
        base_output_path.joinpath(f"{config.logger.wandb.name}_KRegions={k_regions_override}.csv"),
        index=False,
    )
